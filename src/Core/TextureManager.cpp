...
void TextureManager::DrawS(SDL_Texture *tex, const Vector2D &pos, const Vector2D &size, SDL_Rect src) {
	SDL_Rect dst;
	SetPos(dst, pos, size);
	SDL_RenderCopy(Game::renderer, tex, &src, &dst);
}

void TextureManager::DrawEx(SDL_Texture *tex, const Vector2D &pos, const Vector2D &size, const int &angle, SDL_RendererFlip flip) {
	SDL_Rect dst;
	SetPos(dst, pos, size);
